let direction: "up" | "down" | "right" | "left" | "still" = "still";
    ...
    move(direction: "up" | "down" | "right" | "left" | "still"): void {
        switch (direction) {
            case "up":
                ...
                break;
            case "down":
                if (this.position.y > 13) break;
                this.position.y += 1
                break;
            case "right":
                if (this.position.x > 22) break;
                this.position.x += 1
                break;
            case "left":
                if (this.position.x < 1) break;
                this.position.x -= 1
                break;
            case "still":
                // Do nothing
...
class Ball {
    ...
    constructor() {
        this.position = { x: 12, y: 8 }
        this.renderBall()
    }
    htmlBall(): string {
        return `
        <div class="ball" style="top:${this.position.y * 30}px;left:${this.position.x * 30}px" ></div>`
    }
    renderBall() {
        try {
            const elementInHTML = document.getElementById("ballRoot");
            if (!elementInHTML) throw new Error("Element with id 'ballRoot' not found.");
    ...
    moveBall(direction: "up" | "down" | "right" | "left" | "still"): void {
        switch (direction) {
            case "up":
                if (this.position.y < 1) break;
                this.position.y -= 2
...
function directionKey(event): void {
    try {
        const keyMap = {
            "ArrowUp": "up",
            "ArrowDown": "down",
        ...
        direction = keyMap[event];
        if (!direction) {
            direction = "still";
            throw new Error("Invalid key pressed: " + event);
        }
    }
    catch (error) {
        console.error("Error in directionKey function: ", error);
    }
}
...
function goal(): void {
    try {
        ...
        if (!goalElement) throw new Error("Element with id 'goal' not found.");
        goalElement.style.display = "flex";
        setTimeout(() => {
            goalElement.style.display = "none";
            ball.position = { x: 12, y: 8 };
            ball.renderBall();
            messi.position = { x: 3, y: 7 };
            messi.renderPlayer();
        }, 3000);
        this.score++
        renderScore();
...
function renderScore(): void {
    try {
        const scoreElement = document.getElementById("scoreRoot");
        if (!scoreElement) throw new Error("Element with id 'scoreRoot' not found.");
        scoreElement.innerHTML = `Score: ${messi.score}`;
    } catch (error) {
        console.error("Error in renderScore function: ", error);
