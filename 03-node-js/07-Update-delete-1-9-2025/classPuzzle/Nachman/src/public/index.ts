    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const res = await fetch('http://localhost:3000/movies/add-movie', {
                ...
                body: JSON.stringify({ title, year, genre, rating, poster, description, color: '#ffffff' })
            });
...
async function handleColorChange(id: string, newColor: string) {
    try {
        ...
        if (res.ok) {
            ...
            if (movieCard) {
                movieCard.style.backgroundColor = newColor;
            }
        } else {
...
function createMovieCardHTML(movie: Movie): string {
    const backgroundColor = movie.color || '#ffffff';
    
    return `
        <div class="movie-card" data-movie-id="${movie.id}" style="background-color: ${backgroundColor};">
            <h2>${movie.title}</h2>
