...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const genreInput = document.getElementById(
      "movie-genre"
    ) as HTMLInputElement;
...
async function handleRatingChange(id: string, newRating: string) {
  try {
    ...
    const res = await fetch(
      "http://localhost:3000/movies/update-movie-rating",
      {
...
async function handleColorChange(id: string, newColor: string) {
    try {
        console.log('מעדכן צבע:', id, newColor);
        
        const movieCard = document.querySelector(`[data-movie-id="${id}"]`) as HTMLElement;
        if (movieCard) {
            movieCard.style.setProperty('--custom-bg-color', newColor);
            movieCard.classList.add('custom-color');
            console.log('צבע עודכן');
        }
...
function createMovieCardHTML(movie: Movie): string {
  const backgroundColor = movie.color || "white";
  ...
  return `
                ...
                <input type="color" value="${backgroundColor}" onchange="handleColorChange('${movie.id}', this.value)">
                </div>
            `;
}
