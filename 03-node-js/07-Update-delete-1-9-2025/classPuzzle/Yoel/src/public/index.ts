...
async function handleColorChange(id:string, newColor:string){
    try {
        const res = await fetch ('http://localhost:3000/movies/update-color',{
        method : 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
            console.log ("movie color has been successfully updated")
        } else {
            console.error ('failed to update movie rating:', data.error)
        }
        
    } catch (error) {
        console.error ('error occured while updating color', error)
    }
}
...
function createMovieCardHTML(movie: Movie): string {
    return `
                ...
                <input type="color" id="color" name="color" value="${movie.color}">
                <button class="save-movie" onclick="handleSaveMovie('${movie.id}')">Save</button>
                </div>
