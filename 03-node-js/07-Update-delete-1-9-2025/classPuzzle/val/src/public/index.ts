...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const cardColorInput = document.getElementById('movie-card-color') as HTMLInputElement;

        if (!titleInput || !yearInput || !genreInput || !ratingInput || !posterInput || !descriptionInput ||!cardColorInput|| !messageDiv) return;
        ...
        try {
            const res = await fetch('http://localhost:3000/movies/add-movie', {
                ...
                body: JSON.stringify({ title, year, genre, rating, poster, description, cardColor }) //data in string format
            });
...
function createMovieCardHTML(movie: Movie): string {
    const backgroundColor = movie.cardColor || '#3498db';
    return `
                <div class="movie-card" style="background-color: ${backgroundColor};">
                <h2>${movie.title}</h2>
